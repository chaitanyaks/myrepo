...
class AWSDeployEnv():
    ...
    def instanceprofile(self,name,t='ec2'):
...
"""
    ...
    def EC2Ins(self,image_id,instancetype,Keypair,tagkey=None,tagvalue=None,role=None,userdata=None):
...
for i in range(len(data['sertype'])):
    t_res=data['sertype'][i]
    policy = A.policy(t=t_res,name=data['servicetype'][t_res]['policyname'])
    role = A.role(t=t_res,name=data['servicetype'][t_res]['rolename'])
